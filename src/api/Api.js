...
function startExpiration () {
  tokenExpirationInterval = setTimeout(async () => {
    ...
    const results = await Auth.refreshToken()
    console.log(results)
...
export class Auth {
  ...
  static async refreshToken () {
    ...
    if (!(rt = window.localStorage.getItem('refresh_token'))) {
      return {
        succes: false
      }
    }
...
export class User {
  ...
  static async getHomeData () {
    const result = await callApi({ endpoint: '/user/home' })
    if (!result.success) {
...
export class Semester {
  ...
  static async getCompleteSemesterSubjects (semesterID) {
    const url = `/semester/${semesterID}/subjects/complete`
...
export class Subject {
  static async getSubjectsForSemester (semesterID) {
    const url = `/subject/semesterID/${semesterID}`
    return await callApi({ endpoint: url })
  }

  static async getSubjectModules (subjectID) {
    const url = `/subject/${subjectID}/modules`
    return await callApi({ endpoint: url })
  }
}
...
export class ScheduledSubject {
  ...
  static async updateActivityCalif (scheduledSubjectID, activityID, calif) {
    const url = `/scheduledSubject/${scheduledSubjectID}/activityCalif`
    return await callApi({
      endpoint: url,
      method: 'PUT',
      data: {
